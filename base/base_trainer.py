...
class BaseTrainer:
    ...
    def __init__(self, model, loss, metrics, optimizer, config):
        ...
        if self.monitor == 'off':
            ...
            self.metric_stop_value = cfg_trainer.get('metric_stop_value', None)
    ...
    def train(self):
        ...
        Break if validation does not change for early_stop epochs
        """
    ...
    def trainMetricBasedStop(self):
        """
        Full training logic
        Iterates until metric is less than a metric_stop_value
        """
        def isMetricInBound(currentMetric):
            if self.mnt_mode == 'off':
                return False
            elif self.mnt_mode == 'min':
                return currentMetric < self.metric_stop_value
        ...
        epoch_metric = inf if self.mnt_mode == 'min' else 0
        while not isMetricInBound(epoch_metric):
            result = self._train_epoch(epoch)
            ...
            if self.mnt_mode != 'off':
                try:
                    epoch_metric = log[self.mnt_metric]
                    # check whether model performance improved or not, according to specified metric(mnt_metric)
                    improved = (self.mnt_mode == 'min' and log[self.mnt_metric] <= self.mnt_best) or \
                            (self.mnt_mode == 'max' and log[self.mnt_metric] >= self.mnt_best)
                except KeyError:
