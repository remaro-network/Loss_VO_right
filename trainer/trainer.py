...
from utils.util import inf_loop, map_fn, operator_on_dict, LossWrapper, ValueFader
...
class Trainer(BaseTrainer):
    def __init__(self, model, loss, metrics, optimizer, config, data_loader,
        ...
        if self.wrap_loss_in_module:
            self.loss = LossWrapper(loss_function=self.loss, roi=self.roi, device = self.device, options=self.options)
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (data, target) in enumerate(self.data_loader):
            ...
            if not self.module_loss:
                ...
                loss_dict = self.loss(data, self.alpha, self.roi_train,device = self.device, options = self.options)
            else:
