...
@Description: Dataset loaders for AQUALOC Sequence
'''
...
from utils.dataset_utils.tum_utils import associate

from .dataset import Dataset
from utils.general import *

class AQUALOC(Dataset):
    ''' Dataset loader for AQUALOC sequence
    '''
    ...
    def synchronize_timestamps(self):
        ...
        for i,dir in enumerate(rgb_list):
            filename = os.path.basename(os.path.normpath(dir))
            timestamp =int(re.findall(r'\d+', filename)[0])
            aux = {timestamp: dir}
            rgb_dict.update(aux)
    ...
    def get_image(self, timestamp):
        ...
        img_path = os.path.join(self.data_dir['img'], 
                            "frame{:06d}.{}".format(timestamp, self.cfg.image.ext)
                            )
    ...
    def get_gt_poses(self):
        ...
        """
        annotations = os.path.join(self.cfg.directory.gt_pose_dir,'colmap_traj_sequence_'+str(self.cfg.directory.sequence_number)+'.txt')
